// $Id: Bs2JpsiPhi_Signal_v9.cpp,v 1.1 2009/12/06 Pete Clarke Exp $
/** @class Bs2JpsiPhi_Signal_v9 Bs2JpsiPhi_Signal_v9.cpp
 *
...
#include "Bs2JpsiPhi_Signal_v9.h"
...
PDF_CREATOR( Bs2JpsiPhi_Signal_v9 );

//......................................
//Constructor(s)
//New one with configurator
Bs2JpsiPhi_Signal_v9::Bs2JpsiPhi_Signal_v9(PDFConfigurator* configurator) : BasePDF(), 
	// Physics parameters
	gammaName			( configurator->getName("gamma") ), 
	GsGdName			( configurator->getName("GsGd") ),
	...
	_gamma(), GsGd(), dgam(), Aperp_sq(), Apara_sq(), Azero_sq(), As_sq(), delta_para(),
	...
	_useBetaParameter(false), _useMultiplePhis(false), RequireInterference(true)
{
	...
	if( !isCopy ) std::cout << "Constructing PDF: Bs2JpsiPhi_Signal_v9 " << endl;
	...
	_useGsGd = configurator->isTrue( "useGsGd" );
	...
	if( !_useBetaParameter )
	{
		...
		else
		{
			...
			if( !isCopy ) cout << "Bs2JpsiPhi_Signal_v9:: Adding OffsetToGammaForBetaFactor = " << _offsetToGammaForBetaFactor << endl ;
		}
	}
	...
	if( !isCopy )
	{
		if( angAccFile == "" ) cout << "Bs2JpsiPhi_Signal_v9:: Using flat angular acceptance " << endl ;
		else cout << "Bs2JpsiPhi_Signal_v9:: Constructing angAcc using file: " << angAccFile << endl ;
		...
		if( _angAccIgnoreNumerator ) cout << "Bs2JpsiPhi_Signal_v9:: Ignoring angular acceptance numerator " << endl ;
	}
	else
	{
...
//........................................................
//Destructor
Bs2JpsiPhi_Signal_v9::~Bs2JpsiPhi_Signal_v9()
{
...
void Bs2JpsiPhi_Signal_v9::MakePrototypes()
{
	//............................
	...
	parameterNames.push_back( GsGdName );
	...
	//if( _useGsGd ) parameterNames.push_back( GsGdName );
}


//.........................................................
//Return a list of observables not to be integrated
vector<string> Bs2JpsiPhi_Signal_v9::GetDoNotIntegrateList()
{
	vector<string> list;
...
bool Bs2JpsiPhi_Signal_v9::SetPhysicsParameters( ParameterSet* NewParameterSet )
{
	...
	if ( _useGsGd )
	{
		GsGd = allParameters.GetPhysicsParameter( GsGdName )->GetValue() ;
		_gamma += GsGd;
	}
...
*/
		phis_zeroVal = allParameters.GetPhysicsParameter( phis_zeroName )->GetValue();
		phis_paraVal = allParameters.GetPhysicsParameter( phis_paraName )->GetValue() + phis_zeroVal;
		phis_perpVal = allParameters.GetPhysicsParameter( phis_perpName )->GetValue() + phis_zeroVal;
		phis_SVal = allParameters.GetPhysicsParameter( phis_SName )->GetValue() + phis_zeroVal;
		lambda_zeroVal = allParameters.GetPhysicsParameter( lambda_zeroName )->GetValue();
		lambda_paraVal = allParameters.GetPhysicsParameter( lambda_paraName )->GetValue() * lambda_zeroVal;
		lambda_perpVal = allParameters.GetPhysicsParameter( lambda_perpName )->GetValue() * lambda_zeroVal;
		lambda_SVal = allParameters.GetPhysicsParameter( lambda_SName )->GetValue() * lambda_zeroVal;
	}
	else
	{
	...
	//Azero_sq = allParameters.GetPhysicsParameter( Azero_sqName )->GetValue() * (1. + lambda_zeroVal * lambda_zeroVal) / 2.;
	Azero_sq = allParameters.GetPhysicsParameter( Azero_sqName )->GetValue();
	if( (Azero_sq < 0.) || (Azero_sq > 1.)  )
	
	{
		cout << "Warning in Bs2JpsiPhi_Signal_v9::SetPhysicsParameters: Azero_sq <0 or >1 but left as is" <<  endl ;
	}
	//Aperp_sq = allParameters.GetPhysicsParameter( Aperp_sqName )->GetValue() * (1. + lambda_perpVal * lambda_perpVal) / 2.;
	Aperp_sq = allParameters.GetPhysicsParameter( Aperp_sqName )->GetValue();
	if( (Aperp_sq < 0.) || (Aperp_sq > 1.)  )
	{
		cout << "Warning in Bs2JpsiPhi_Signal_v9::SetPhysicsParameters: Aperp_sq <0 or >1 but left as is" <<  endl;
	}
	...
	if( Apara_sq < 0. )
	{
		cout << "Warning in Bs2JpsiPhi_Signal_v9::SetPhysicsParameters: derived parameter Apara_sq <0  and so set to zero" <<  endl ;
		Apara_sq = 0. ;
	}
	...
	if( (fs < 0.) || (fs >= 1.) )
	{
		cout << "Warning in Bs2JpsiPhi_Signal_v9::SetPhysicsParameters: As_sq <0 or >=1 but left as is" <<  endl ;
	}

	As_sq = fs / (1. - fs );
	//As_sq = As_sq * (1. + lambda_SVal * lambda_SVal) / 2.;
...
double Bs2JpsiPhi_Signal_v9::EvaluateForNumericIntegral(DataPoint * measurement)
{
...
double Bs2JpsiPhi_Signal_v9::Evaluate(DataPoint * measurement)
{
	...
	if( !performingComponentProjection )
	{
		if( DebugFlag_v8 )
		{
			...
			if( c1 || c3 )
			{
				this->DebugPrint( " Bs2JpsiPhi_Signal_v9::Evaluate() returns <=0 or nan :" , returnValue ) ;
...
double Bs2JpsiPhi_Signal_v9::EvaluateTimeOnly(DataPoint * measurement)
{
	_datapoint = measurement;
	...
	if( DebugFlag_v8 )
	{
		...
		if( (c1 || c3)  )
		{
			this->DebugPrintEvaluate( " Bs2JpsiPhi_Signal_v9::EvaluateTimeOnly() returns <=0 or nan :" , returnValue ) ;
			if( std::isnan(returnValue) ) throw 10 ;
			if( returnValue <= 0. ) throw 10 ;
		}
	}
...
double Bs2JpsiPhi_Signal_v9::Normalisation(DataPoint * measurement, PhaseSpaceBoundary * boundary)
{
	_datapoint = measurement;
	...
	if( !performingComponentProjection )
	{
		if( DebugFlag_v8 )
		{
			...
			if( c1 || c2 )
			{
				this->DebugPrintNormalisation( " Bs2JpsiPhi_Signal_v9::Normalisation() returns <=0 or nan :" , returnValue ) ;
				if( std::isnan(returnValue) ) throw 10 ;
				if( returnValue <= 0. ) throw 10 ;
			}
		}
...
void Bs2JpsiPhi_Signal_v9::preCalculateTimeFactors()
{
...
//.......................................................
// Pre calculate the time integrals : this is becaue these functions are called many times for each event due to the 10 angular terms
void Bs2JpsiPhi_Signal_v9::preCalculateTimeIntegrals()
{
	intExpL_stored = resolutionModel->ExpInt( tlo, thi, gamma_l() );
	intExpH_stored = resolutionModel->ExpInt( tlo, thi, gamma_h() );
}

void Bs2JpsiPhi_Signal_v9::preCalculateSinusoidIntegrals()
{
	if( _eventIsTagged && RequireInterference )
	{
...
void Bs2JpsiPhi_Signal_v9::preCalculateTimeIntegralsDebug() const
{
...
vector<string> Bs2JpsiPhi_Signal_v9::PDFComponents()
{
...
double Bs2JpsiPhi_Signal_v9::EvaluateComponent( DataPoint* input, ComponentRef* Component )
{
...
double Bs2JpsiPhi_Signal_v9::diffXsec()
{
	...
	if( DebugFlag_v8 )
	{
		if( xsec < 0)
		{
			this->DebugPrintXsec( " Bs2JpsiPhi_Signal_v9_v1::diffXsec( ) : return value < 0 = ", xsec );
		}
	}
...
double Bs2JpsiPhi_Signal_v9::diffXsecTimeOnly()
{
	preCalculateTimeFactors() ;
	...
	if( DebugFlag_v8 )
	{
		if( xsec < 0 )
		{
			this->DebugPrintXsec( " Bs2JpsiPhi_Signal_v9_v1::diffXsecTimeOnly( ) : return value < 0 = ", xsec );
		}
	}
...
double Bs2JpsiPhi_Signal_v9::diffXsecNorm1()
{
	double norm =
	...
	if( DebugFlag_v8 )
	{
		if( norm < 0 )
		{
			this->DebugPrintNorm( " Bs2JpsiPhi_Signal_v9_v1::diffXsecNorm1( ) : return value < 0 = ", norm );
		}
	}
	return norm ;
}

void Bs2JpsiPhi_Signal_v9::generateTimeIntegrals()
{
	this->preCalculateTimeIntegrals();
}

void Bs2JpsiPhi_Signal_v9::generateSinusoidIntegrals()
{
	this->preCalculateSinusoidIntegrals();
}

void Bs2JpsiPhi_Signal_v9::ConstructTimeIntegrals()
{
...
double Bs2JpsiPhi_Signal_v9::diffXsecCompositeNorm1(  )
{
...
void Bs2JpsiPhi_Signal_v9::CacheAmplitudesAndAngles()
{
...
void Bs2JpsiPhi_Signal_v9::prepareCDS( double lambdaVal, double PhisVal )
{
...
double Bs2JpsiPhi_Signal_v9::calculateTimeFactor( double ak, double bk, double ck, double dk )
{
        if( _eventIsTagged )
        {
                return  
			_mistagCalibModel->D1() * ( ak * ( Expcosh_dGt() ) + bk * ( Expsinh_dGt() ) ) +
                        _mistagCalibModel->D2() * 2.0 * ( ck * expCos( ) + dk * expSin( ) );
        }
        else
        {
                return ak * ( Expcosh_dGt() ) + bk * ( Expsinh_dGt() );
        }
}

double Bs2JpsiPhi_Signal_v9::calculateTimeFactorInt( double ak, double bk, double ck, double dk )
{
        if( _eventIsTagged )
        {
                return  
			_mistagCalibModel->D1() * ( ak * ( int_Expcosh_dGt() ) + bk * ( int_Expsinh_dGt() ) ) +
                        _mistagCalibModel->D2() * 2.0 * ( ck * intExpCos( ) + dk * intExpSin( ) );
        }
        else
        {
                return ak * ( int_Expcosh_dGt() ) + bk * ( int_Expsinh_dGt() );
        }
}

void Bs2JpsiPhi_Signal_v9::DebugPrintEvaluate( string message, double value )
{
	PDF_THREAD_LOCK
...
void Bs2JpsiPhi_Signal_v9::DebugPrintNormalisation( string message, double value )
{
	PDF_THREAD_LOCK
...
void Bs2JpsiPhi_Signal_v9::DebugPrint( string message, double value )  const
{
	if( !performingComponentProjection )
	{
		(void) message; (void) value;
		cout << "*************DEBUG OUTPUT FROM Bs2JpsiPhi_Signal_v9::DebugPrint ***************************" << endl ;
...
void Bs2JpsiPhi_Signal_v9::DebugPrintXsec( string message, double value )
{
	PDF_THREAD_LOCK
		...
		{
			(void) message; (void) value;
			cout << "*************DEBUG OUTPUT FROM Bs2JpsiPhi_Signal_v9::DebugPrintXsec ***************************" << endl;
			cout << message << value << endl << endl;
...
void Bs2JpsiPhi_Signal_v9::DebugPrintNorm( string message, double value )
{
	PDF_THREAD_LOCK
		...
		{
			(void) message; (void) value;
			cout << "*************DEBUG OUTPUT FROM Bs2JpsiPhi_Signal_v9::DebugPrintNorm ***************************" << endl;
			cout << message << value << endl << endl;
...
double Bs2JpsiPhi_Signal_v9::timeFactorEven()  const
{
	if( _eventIsTagged )
	{
		return
	...
	{
		return ( 1.0 + DD() ) * expL() + ( 1.0 - DD() ) * expH();
	}
}

void Bs2JpsiPhi_Signal_v9::timeFactorEvenDebug() const
{
	cout << "D1: " << _mistagCalibModel->D1() << " * ( " << ( 1.0 + DD() ) * expL( ) << " + " << ( 1.0 + DD() ) * expH( ) << " )" << endl;
	cout << "D2: " << _mistagCalibModel->D2() << " * ( " << ( 2.0 * SS() ) * expSin( ) << " + " << + ( 2.0 * CC()      ) * expCos( ) << " )" << endl;
}

double Bs2JpsiPhi_Signal_v9::timeFactorEvenInt()  const
{
	if( _eventIsTagged )
	{
		return
	...
	{
		return ( 1.0 + DD() ) * intExpL() + ( 1.0 - DD() ) * intExpH();
	}
}


void Bs2JpsiPhi_Signal_v9::timeFactorEvenIntDebug() const
{
	...
	cout << "   " << _mistagCalibModel->D1() *( ( 1.0 + DD() )  * intExpL() + ( 1.0 - DD() )  * intExpH() ) << endl;
	cout << " + " << _mistagCalibModel->D2() *( ( 2.0 * SS() ) * intExpSin( ) + ( 2.0 * CC()      ) * intExpCos( ) )<< endl;
}


double Bs2JpsiPhi_Signal_v9::timeFactorOdd(  )   const
{
	if( _eventIsTagged )
	{
		return
...
double Bs2JpsiPhi_Signal_v9::timeFactorOddInt(  )  const
{
	if( _eventIsTagged )
	{
		return
...
double Bs2JpsiPhi_Signal_v9::timeFactorA0A0( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorA0A0Int( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorAPAP( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorAPAPInt( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorATAT( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorATATInt( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorImAPAT( )
{
	double ak, bk, ck, dk;
	...
	* sin(delta_perp - delta_para - phis_perpVal + phis_paraVal));
 	...
 	* sin(delta_perp - delta_para - phis_perpVal + phis_paraVal));
	dk = - 1./ (2. * xk)  *(std::abs(lambda_perpVal) * cos(delta_perp - delta_para - phis_perpVal) 
	+ std::abs(lambda_paraVal) * cos(delta_para - delta_perp - phis_paraVal));

	return xk * calculateTimeFactor( ak, bk, ck, dk );
}

double Bs2JpsiPhi_Signal_v9::timeFactorImAPATInt( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorReA0AP( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorReA0APInt( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorImA0AT(  )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorImA0ATInt( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorASAS( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorASASInt( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorReASAP( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorReASAPInt( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorImASAT( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorImASATInt( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorReASA0( )
{
	double ak, bk, ck, dk;
...
double Bs2JpsiPhi_Signal_v9::timeFactorReASA0Int( )
{
	double ak, bk, ck, dk;
